  ...
  Admin-only. Used for mockup generation presets.
              ...
              <div class="tab-pane fade" id="tab_variants">
                ...
                <div class="table-responsive">
                  <table class="table table-bordered" id="variantsTable">
                    ...
                    <tbody>
                      @php $oldVariants = old('variants', [['sku'=>'','color_name'=>'','color_hex'=>'','size'=>'','stock_count'=>'','additional_cost'=>'']]); @endphp
    @foreach($oldVariants as $i => $v)
      <tr>
        ...
        <td>
          <input type="text" name="variants[{{ $i }}][color_name]" class="form-control" value="{{ $v['color_name'] }}" placeholder="Black">
        </td>
        <td>
          <input type="text" name="variants[{{ $i }}][color_hex]" class="form-control @error('variants.$i.color_hex') is-invalid @enderror"
                 value="{{ $v['color_hex'] }}" placeholder="#000000">
          @error("variants.$i.color_hex")
            <div class="invalid-feedback">{{ $message }}</div>
          @enderror
        </td>
        <td>
          <input type="text" name="variants[{{ $i }}][size]" class="form-control" value="{{ $v['size'] }}" placeholder="M">
        </td>
        <td>
          <input type="number" name="variants[{{ $i }}][stock_count]" class="form-control" value="{{ $v['stock_count'] }}" placeholder="-1">
        </td>
        <td>
          <input type="number" name="variants[{{ $i }}][additional_cost]" class="form-control" step="0.01" value="{{ $v['additional_cost'] }}" placeholder="0.00">
        </td>
...
@enderror
                  ...
                  <table class="table table-bordered" id="imagesTable">
                    <thead>
                      <tr>
                        ...
                        @php $oldImages = old('images', [['image_url'=>'','image_type'=>'mockup','angle'=>'','alt_text'=>'']]); @endphp
    @foreach($oldImages as $i => $img)
      <tr>
        ...
        <td>
          <select name="images[{{ $i }}][image_type]" class="form-control">
            @foreach(['mockup','preview','lifestyle'] as $type)
              <option value="{{ $type }}" {{ $img['image_type']==$type ? 'selected' : '' }}>{{ $type }}</option>
            @endforeach
          </select>
        </td>
        <td>
          <input type="text" name="images[{{ $i }}][angle]" class="form-control" value="{{ $img['angle'] }}" placeholder="front">
        </td>
        <td>
          <input type="text" name="images[{{ $i }}][alt_text]" class="form-control" value="{{ $img['alt_text'] }}" placeholder="Front mockup">
        </td>
        <td class="text-center">
          <button type="button" class="btn btn-sm bg-soft-danger text-danger removeRow">Remove</button>
        </td>
...
<script>
let variantIndex = document.querySelectorAll('#variantsTable tbody tr').length;
let imageIndex = document.querySelectorAll('#imagesTable tbody tr').length;

// ===== ADD VARIANT =====
document.body.addEventListener('click', function(e) {
    if (e.target.id === 'addVariant') {
        ...
        row.innerHTML = `
            ...
            <td><input class="form-control" name="variants[${variantIndex}][stock_count]" type="number" value="-1"></td>
            <td><input class="form-control" name="variants[${variantIndex}][additional_cost]" type="number" step="0.01" value="0"></td>
            <td class="text-center">
                <button type="button" class="btn btn-sm bg-soft-danger text-danger removeRow">Remove</button>
            </td>
        `;
...
document.getElementById('addImage').addEventListener('click', function () {
    ...
    row.innerHTML = `
        ...
        <td><input class="form-control" name="images[${imageIndex}][angle]"></td>
        <td><input class="form-control" name="images[${imageIndex}][alt_text]"></td>
        <td class="text-center">
            <button type="button" class="btn btn-sm bg-soft-danger text-danger removeRow">Remove</button>
        </td>
    `;
...
document.body.addEventListener('click', function(e) {
    ...
    if (table === 'variantsTable') reindexVariants();
    if (table === 'imagesTable') reindexImages();
});

// ===== REINDEX VARIANTS =====
function reindexVariants() {
    document.querySelectorAll('#variantsTable tbody tr').forEach((row, index) => {
        row.querySelectorAll('input').forEach(input => {
            input.name = input.name.replace(/variants\[\d+\]/, `variants[${index}]`);
        });
    });
    variantIndex = document.querySelectorAll('#variantsTable tbody tr').length;
    validateSKUs();
}

// ===== REINDEX IMAGES =====
function reindexImages() {
    document.querySelectorAll('#imagesTable tbody tr').forEach((row, index) => {
        row.querySelectorAll('input, select').forEach(input => {
            input.name = input.name.replace(/images\[\d+\]/, `images[${index}]`);
        });
    });
    imageIndex = document.querySelectorAll('#imagesTable tbody tr').length;
}

// ===== SKU VALIDATION =====
document.addEventListener('input', function(e) {
    if (!e.target.name.includes('[sku]')) return;
    validateSKUs();
});

function validateSKUs() {
    const inputs = [...document.querySelectorAll('input[name*="[sku]"]')];
    const skus = inputs.map(i => i.value.trim()).filter(Boolean);
...
document.querySelector('form').addEventListener('submit', function (e) {
    const duplicateSkus = document.querySelectorAll(
        'input[name*="[sku]"].is-invalid'
    );

    if (duplicateSkus.length > 0) {
        e.preventDefault();
        alert('Duplicate SKUs found. Please fix them before saving.');
        duplicateSkus[0].focus();
    }
});
